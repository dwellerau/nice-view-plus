...
/* Forward declaration of the CAPS widget init (from zmk-feature-caps) */
#if !IS_ENABLED(CONFIG_ZMK_WIDGET_WPM_STATUS)
...
#if IS_ENABLED(CONFIG_ZMK_WIDGET_WPM_STATUS)
    // Draw WPM area + numeric + sparkline
...
#if !IS_ENABLED(CONFIG_ZMK_WIDGET_WPM_STATUS)
    /* Instantiate CAPS widget in place of WPM block:
     * - parent to the container (so canvas drawing doesn’t hide it),
     * - align to TOP canvas at (1, 22),
     * - size 66x40,
     * - transparent background/border/padding,
     * - bring to foreground.
     */
    if (top) {
