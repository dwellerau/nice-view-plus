...
/* >>> ADDED: forward declaration generated by
 * ZMK_DISPLAY_WIDGET_INIT_FN(caps_status, detect_mode)
 * in zmk-feature-caps. We only need it when WPM is off. */
#if !IS_ENABLED(CONFIG_ZMK_WIDGET_WPM_STATUS)
...
int zmk_widget_status_init(struct zmk_widget_status *widget, lv_obj_t *parent) {
    ...
    /* >>> ADDED: create the caps_status widget in the WPM slot when WPM is disabled.
     * Parent it to the TOP canvas so its coordinates (1,22 .. 66x40) match the WPM inner box. */
#if !IS_ENABLED(CONFIG_ZMK_WIDGET_WPM_STATUS)
    if (top) {
        lv_obj_t *caps = zmk_widget_caps_status_init(top);
        /* Inner WPM rectangle geometry: (1,22) with size (66x40) */
        lv_obj_set_pos(caps, 1, 22);
